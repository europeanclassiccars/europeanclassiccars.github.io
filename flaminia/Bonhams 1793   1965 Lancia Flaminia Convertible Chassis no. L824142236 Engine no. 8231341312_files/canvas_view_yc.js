/*
	...
	Last Changed:  $Date: 2012-09-18 16:51:23 +0100 (Tue, 18 Sep 2012) $ $Rev: 878 $ by $Author: matt $
...
*/
Math.parseInt=function(b,a){if(!b||!(/^[-]*[\d]+$/.test(b))){return a;}else{return parseInt(b,10);}};
Math.parseFloat=function(b,a){if(!b||!(/^[-]*[.|\d]+$/.test(b))){return a;}else{return parseFloat(b);
}};Math.round1=function(a){return Math.round(a*10)/10;};Math.round8=function(a){return Math.round(a*Math.pow(10,8))/Math.pow(10,8);
...
};function ImgGrid(b,j,d,c,a,f,g,i){this.initialised=false;this.useJSONP=g;this.onInitialisedFn=i;this.animating=false;
this.excanvas=(Browser.ie&&window.G_vmlCanvasManager);this.g2d={ctx:c,origin:{x:0,y:0}};var e=Math.linearTween;
switch(a.toLowerCase()){case"in-out-back":e=Math.easeInOutBack;break;case"in-out-quadratic":e=Math.easeInOutQuad;
...
break;case"out-sine":e=Math.easeOutSine;break;}this.zoom={level:1,nextLevel:1,maxLevel:10,drawZoom:{x:1,y:1},factor:0.5,fps:50,animateFn:e};
...
d=d.cleanQueryString().replace(/\+/g," ");var h=d.indexOf("?");this.urlParams=d.substring(h+1).parseQueryString();
d=d.substring(0,h);h=d.lastIndexOf("/");this.urlBase=d.substring(0,h+1);this.urlCommand=d.substring(h+1);
this.loadingText="Loading image...";this.drawText(this.loadingText);this.loadImageInfo();}ImgGrid.prototype.destroy=function(){this.cancelPendingImages();
this.initialised=false;this.g2d.ctx=null;};ImgGrid.prototype.reset=function(){if(!this.initialised){return;
...
}else{this.drawText(this.loadingText);if(this.grids.length>0){this.centreGrid(true,true,false);this.setGrid(this.zoom.level,false);
}}};ImgGrid.prototype.loadImageInfo=function(){var a=this.urlBase+"api/details?src="+this.urlParams.src;
...
}.bind(this)}).get();}};ImgGrid.prototype.onImageInfoLoaded=function(i){if((i.status>=300)||(i.data.width<=0)||(i.data.height<=0)){this.onImageInfoFailure(null);
}else{var g=i.data.width,f=i.data.height;if(this.urlParams.top||this.urlParams.left||this.urlParams.right||this.urlParams.bottom){var e=Math.limit(Math.parseFloat(this.urlParams.top,0),0,1),c=Math.limit(Math.parseFloat(this.urlParams.left,0),0,1),h=Math.limit(Math.parseFloat(this.urlParams.right,1),0,1),a=Math.limit(Math.parseFloat(this.urlParams.bottom,1),0,1);
if((e<a)&&(c<h)){g=Math.round(g*(h-c));f=Math.round(f*(a-e));}}if(this.urlParams.angle){var d=Math.abs(Math.parseInt(this.urlParams.angle,0));
if((d==90)||(d==270)){var b=g;g=f;f=b;}}this.imageInfo=i.data;this.initialise(g,f);}};ImgGrid.prototype.onImageInfoFailure=function(a){this.drawText("X");
};ImgGrid.prototype.initialise=function(d,c){this.gridOpts.maxWidth=d;this.gridOpts.maxHeight=c;this.gridOpts.aspect=d/c;
if(this.gridOpts.aspect>=this.viewport.origAspect){this.gridOpts.minWidth=Math.min(d,this.viewport.origWidth);
...
if(a.max){this.zoom.maxLevel=b;break;}}this.setGrid(1,false);this.requestImage(1,1);};ImgGrid.prototype.closestRatioMultiples=function(a,l,m,i,g){var c=Math.roundToMultiple(a,g),k=Math.roundToMultiple(c/i,g);
if((c/k==i)&&(c>=l)&&(c<=m)){return{a:c,b:k};}else{var f=Math.roundToMultiple(l,g),e=Math.roundToMultiple(m,g),h=new Array();
if(f<l){f+=g;}if(e>m){e-=g;}for(var b=f;b<=e;b+=g){var j=Math.roundToMultiple(b/i,g),d=b/j;if(d==i){return{a:b,b:j};
}else{h.push({a:b,b:j,diff:Math.abs(i-d)});}}if(h.length==0){return{a:c,b:k};}else{h.sort(function(o,n){return o.diff-n.diff;
});return h[0];}}};ImgGrid.prototype.calcGridSize=function(f){var g=(1+((f-1)*this.zoom.factor))*Math.max(1,Math.pow(1.3,(f-2))),k=Math.floor(this.gridOpts.minWidth*g),a=Math.round(k/this.gridOpts.aspect);
var m=false;if((k/this.gridOpts.maxWidth)>0.85){k=this.gridOpts.maxWidth;a=this.gridOpts.maxHeight;m=true;
}var e=Math.floor(Math.max(k/(this.gridOpts.minWidth*1.2),a/(this.gridOpts.minHeight*1.2)));var d=Math.limit(e*e,1,this.gridOpts.maxTiles);
var p=[1,4,16,64,256,1024];for(var j=0;j<p.length;j++){if(p[j]>=d){d=p[j];break;}}var n=Math.round(Math.sqrt(d)),c=Math.max(n,4),h=(f==1)?0:10,b=Math.max(this.gridOpts.minWidth,k-(h*c)),o=Math.min(this.gridOpts.maxWidth,k+(h*c));
var l=this.closestRatioMultiples(k,b,o,this.gridOpts.aspect,c);k=l.a;a=l.b;return{width:k,height:a,length:d,axis:n,max:m};
...
this.urlParams.autosizefit="0";this.urlParams.strip="1";this.urlParams.nostats=(a>1)?"1":"0";this.urlParams.width=this.grids[a].origWidth;
...
}.bind(this),10);}}if((a==this.zoom.level)&&!this.animating){this.paint();}};ImgGrid.prototype.cancelPendingHiddenImages=function(){var a=this.getVisibleGridTiles();
...
this.alignGrid(true);this.cancelPendingHiddenImages();};ImgGrid.prototype.drawText=function(c){var b=(this.viewport.width/c.length)*0.66;
var a=this.g2d.ctx;a.save();a.font=b+"pt Arial";a.fillStyle="#aaaaaa";a.textAlign="center";a.textBaseline="middle";
this.clear();a.fillText(c,this.viewport.width/2,this.viewport.height/2);a.restore();};ImgGrid.prototype.clear=function(){this.g2d.ctx.clearRect(-this.g2d.origin.x,-this.g2d.origin.y,this.viewport.width,this.viewport.height);
...
}a.restore();};function ImgCanvasView(b,c,d){this.options={title:null,description:null,showcontrols:"auto",quality:true,animation:"out-quadratic",maxtiles:256,jsonp:true,doubleclickreset:true,controls:{title:true,help:true,reset:true,fullscreen:!Browser.ie6,zoomin:true,zoomout:true},fullScreenFixed:true,fullScreenCloseOnClick:true};
if(d!=undefined){var a=this.options.controls;Object.append(this.options,d);if(d.controls!=undefined){Object.append(a,d.controls);
this.options.controls=a;}}this.uiAttrs={controlsSlider:null,alertVisible:false,alertEl:null,fullScreen:false,fullMaskEl:null,fullSwapEl:null,fullKeydownFn:null,fullResizeFn:null,animating:false};
...
this.imageInfo=null;this.ctrEl=$(b);this.ctrEl.empty();this.canvas=new Element("canvas",{width:1,height:1,styles:{"-webkit-user-select":"none","-khtml-user-select":"none","-moz-user-select":"none","-o-user-select":"none","user-select":"none","-webkit-tap-highlight-color":"rgba(0,0,0,0)","-webkit-touch-callout":"none"}});
...
}}this.content=new ImgGrid(this.canvas.width,this.canvas.height,c,this.canvasContext,this.options.animation,this.options.maxtiles,this.options.jsonp,function(e){this.onContentReady(e);
}.bind(this));if(this.options.showcontrols!="no"){this.createControls();}}ImgCanvasView.prototype.destroy=function(){this.content.destroy();
this.content=null;this.canvas.destroy();this.canvas=null;this.canvasContext=null;};ImgCanvasView.prototype.init=function(){this.canvas.removeEvents();
...
});this.canvas.addEvent("dragstart",function(a){return false;});};ImgCanvasView.prototype.layout=function(){this.ctrOuterPos=this.ctrEl.getCoordinates();
this.ctrInnerPos=this.ctrEl.getComputedSize();if((this.ctrInnerPos.width==0)&&(this.ctrInnerPos.height==0)){this.ctrInnerPos={width:this.ctrEl.clientWidth,height:this.ctrEl.clientHeight};
...
}this.canvas.width=this.ctrInnerPos.width;this.canvas.height=this.ctrInnerPos.height;if(this.content!=undefined){this.content.setViewportSize(this.canvas.width,this.canvas.height);
}};ImgCanvasView.prototype.reset=function(){this.content.reset();this.refreshZoomControls();};ImgCanvasView.prototype.onMouseDown=function(a){if(!a.rightClick){if((a.api_event==undefined)&&this.options.doubleclickreset&&(Date.now()-this.mouseAttrs.downTime<300)){this.reset();
...
if(this.content.initialised&&!this.content.animating){setTimeout(function(){var c=(a.page.x-this.mouseAttrs.down_x);
...
if(c.x<0||c.x>1||c.y<0||c.y>1){this.uiAttrs.fullMaskEl.fireEvent("click");return;}}if(this.content.initialised&&!this.content.animating){if(!this.mouseAttrs.dragged){var c=this.getClickPosition(d,true);
...
};ImgCanvasView.prototype.onContentReady=function(a){if(this.options.title!=null){a.title=this.options.title;
}if(this.options.description!=null){a.description=this.options.description;}this.imageInfo=a;this.setImageTitle(a.title);
this.refreshZoomControls();if(this.uiAttrs.fullScreen){this.autoZoomFit();}};ImgCanvasView.prototype.autoZoomFit=function(){if(this.content.initialised){this.content.zoomFit();
...
}else{return e;}};ImgCanvasView.prototype.createControls=function(){if(this.options.showcontrols=="auto"){var g=new Element("div",{"class":"controltoggle panelbg",html:"&nbsp;"});
g.addEvent("mousedown",function(){this.toggleControls();}.bind(this));this.ctrEl.grab(g);}var h=new Element("div",{styles:{width:"100%","line-height":"normal","text-align":"center",cursor:"default",visibility:(this.options.showcontrols=="auto")?"hidden":"visible"},events:{click:function(j){if(this.uiAttrs.fullMaskEl!=null){this.uiAttrs.fullMaskEl.fireEvent("click");
}}.bind(this)}});var i=new Element("span",{"class":"controlpanel panelbg",events:{click:function(j){j.stopPropagation();
}}});h.grab(i);if(this.options.controls.title){var c=new Element("span",{"class":"controltitle",html:"Loading..."});
i.grab(c);}if(this.options.controls.help){var a=new Element("span",{"class":"icon help",title:"Help",html:"&nbsp;"});
a.addEvent("mousedown",function(){this.toggleHelp();}.bind(this));i.grab(a);}if(this.options.controls.reset){var e=new Element("span",{"class":"icon reset",title:"Reset zoom",html:"&nbsp;"});
e.addEvent("mousedown",function(){this.reset();}.bind(this));i.grab(e);}if(this.options.controls.zoomin){var f=new Element("span",{"class":"icon zoomin",title:"Zoom in",html:"&nbsp;"});
f.addEvent("mousedown",function(){this.autoZoom(true);}.bind(this));i.grab(f);}if(this.options.controls.zoomout){var d=new Element("span",{"class":"icon zoomout",title:"Zoom out",html:"&nbsp;"});
d.addEvent("mousedown",function(){this.autoZoom(false);}.bind(this));i.grab(d);}if(this.options.controls.fullscreen){var b=new Element("span",{"class":"icon fulltoggle",title:"Toggle full screen mode",html:"&nbsp;"});
b.addEvent("mousedown",function(){this.toggleFullscreen();}.bind(this));i.grab(b);}this.controlpanel=h;
this.ctrEl.grab(this.controlpanel);this.controlpanel.getElements(".icon").each(function(j){j.addEvent("mouseover",function(){j.addClass("rollover");
});j.addEvent("mouseout",function(){j.removeClass("rollover");});});if(this.options.showcontrols=="auto"){this.uiAttrs.controlsSlider=new Fx.Slide(h,{onComplete:function(){var j=this.uiAttrs.controlsSlider.open;
if(g){j?g.removeClass("up"):g.addClass("up");}}.bind(this)});this.uiAttrs.controlsSlider.hide();h.setStyle("visibility","visible");
...
});}};ImgCanvasView.prototype.refreshZoomControls=function(){if(this.content.initialised&&this.controlpanel){var e=this.controlpanel.getElement(".zoomin"),a=this.controlpanel.getElement(".zoomout"),d=this.controlpanel.getElement(".reset"),c=(this.content.zoom.nextLevel<this.content.zoom.maxLevel),b=(this.content.zoom.nextLevel>1),f=b;
if(e){c?e.removeClass("disabled"):e.addClass("disabled");}if(a){b?a.removeClass("disabled"):a.addClass("disabled");
}if(d){f?d.removeClass("disabled"):d.addClass("disabled");}}};ImgCanvasView.prototype.setImageTitle=function(c){if(this.controlpanel){var a=this.controlpanel.getElement(".controltitle");
...
a.removeEvents("click");a.addEvent("click",function(){this.toggleImageInfo();}.bind(this));}}};ImgCanvasView.prototype.toggleControls=function(){if(this.controlpanel&&this.uiAttrs.controlsSlider){this.clearRollovers();
this.refreshZoomControls();this.uiAttrs.controlsSlider.toggle();}};ImgCanvasView.prototype.toggleAlert=function(d){if(this.uiAttrs.alertVisible){this.uiAttrs.alertEl.destroy();
this.uiAttrs.alertEl=null;this.uiAttrs.alertVisible=false;}else{d=d.replace(/\r\n?/g,"\n");d=d.replace(/\n/g,"<br/>");
this.uiAttrs.alertEl=new Element("div",{styles:{position:"absolute",width:"0px",height:"0px","z-index":"1002"}});
var c=new Element("div",{"class":"alertpanel panelbg",html:this.stripTags(d,"(?!br)"),styles:{position:"absolute","z-index":"1002","line-height":"normal",overflow:"auto",visibility:"hidden"},events:{mousedown:function(){this.toggleAlert();
...
}else{var a="No information available";}this.toggleAlert(a);};ImgCanvasView.prototype.toggleFullscreen=function(){if(Browser.ie6){return;
}if(this.uiAttrs.animating){return;}if(this.content.animating){return;}if(this.uiAttrs.alertVisible){this.toggleAlert();
}if(this.uiAttrs.fullResizeFn==null){this.uiAttrs.fullKeydownFn=function(b){this.fullscreenKeydown(b);
...
window.removeEvent("keydown",this.uiAttrs.fullKeydownFn);this.ctrEl.dispose();this.ctrEl.setStyles(this.uiAttrs.containerStyles);
this.ctrEl.removeClass("fullscreen");this.ctrEl.replaces(this.uiAttrs.fullSwapEl);this.uiAttrs.fullSwapEl.destroy();
this.uiAttrs.fullSwapEl=null;this.uiAttrs.fullMaskEl.destroy();this.uiAttrs.fullMaskEl=null;this.layout();
this.clearRollovers();this.reset();this.uiAttrs.animating=false;this.uiAttrs.fullScreen=false;}.bind(this)}).start("opacity",1,0);
}else{var a=this.fullscreenGetCoords();this.uiAttrs.containerStyles=this.ctrEl.getStyles("position","z-index","opacity","left","top","width","height","margin");
this.uiAttrs.fullMaskEl=new Mask(document.body,{hideOnClick:false,"class":"fullscreen_mask",style:{"z-index":"1000"},onClick:function(){this.toggleFullscreen();
}.bind(this)});this.uiAttrs.fullMaskEl.show();this.uiAttrs.fullSwapEl=this.ctrEl.clone(false,true);this.uiAttrs.fullSwapEl.replaces(this.ctrEl);
this.ctrEl.setStyles({position:this.options.fullScreenFixed?"fixed":"absolute","z-index":"1001",opacity:"0",left:a.left+"px",top:a.top+"px",width:a.width+"px",height:a.height+"px",margin:"0"});
this.ctrEl.addClass("fullscreen");$(document.body).grab(this.ctrEl,"top");this.layout();this.clearRollovers();
window.addEvent("keydown",this.uiAttrs.fullKeydownFn);window.addEvent("resize",this.uiAttrs.fullResizeFn);
new Fx.Tween(this.ctrEl,{duration:500,onComplete:function(){this.autoZoomFit();}.bind(this)}).start("opacity",0,1);
this.uiAttrs.fullScreen=true;}};ImgCanvasView.prototype.fullscreenGetCoords=function(){var c=window.getSize(),a=this.options.fullScreenFixed?{x:0,y:0}:window.getScroll(),f=Math.min(Math.round(c.x/20),Math.round(c.y/20));
...
};ImgCanvasView.prototype.fullscreenKeydown=function(a){if(a.code==27){setTimeout(function(){this.toggleFullscreen();
}.bind(this),1);}};ImgCanvasView.prototype.fullscreenResize=function(){var a=this.fullscreenGetCoords();
this.ctrEl.setStyles({left:a.left+"px",top:a.top+"px",width:a.width+"px",height:a.height+"px"});this.layout();
};function _img_fs_zoom_click(c,b){if(Browser.ie6){return;}var a=$("_img_fs_zoom_click_el");if(!a){a=new Element("div",{id:"_img_fs_zoom_click_el","class":"imageviewer",styles:{position:"absolute",display:"block",width:"500px",height:"500px",left:"-1000px"}});
$(document.body).grab(a,"top");}canvas_view_init(a,c.src,b);canvas_view_toggle_fullscreen(a);}function _get_ct_viewer(a){a=$(a);
return(a&&a._viewer)?a._viewer:null;}var _hcvs=null;function haveCanvasSupport(){if(_hcvs==null){var a=new Element("canvas");
if(Browser.ie&&window.G_vmlCanvasManager){G_vmlCanvasManager.initElement(a);}_hcvs=(a&&a.getContext);
}return _hcvs;}function canvas_view_init(a,c,b){a=$(a);if(a){if(haveCanvasSupport()){if(a._viewer!=undefined){a._viewer.destroy();
}var d=new ImgCanvasView(a,c,b);d.init();a._viewer=d;}else{a.innerHTML="Sorry, this control is unsupported. Try upgrading your web browser.";
...
}return false;}function canvas_view_init_image(c,b){c=$(c);if(c){var a=c.title||c.alt;if(a){if(b==undefined){var b={};
}if(b.title==undefined){b=Object.clone(b);b.title=a;}}c.removeEvents("click");c.addEvent("click",function(){_img_fs_zoom_click(c,b);
});}return false;}function canvas_view_init_all_images(b,a){$$("."+b).each(function(c){canvas_view_init_image(c,a);
});return false;}